...
for i in "${!allFileNames[@]}"; do
    cd "training_outcome_"${allFileNames[$i]}"_comb/"
    cp ../training_outcome_20_comb/find_limit_with_loop2.C ./find_limit_with_loop2.C
    root -q -l -x "find_limit_with_loop2.C(\"${allFileNames[$i]}\")"
    cd ..
done